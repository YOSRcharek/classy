...
console.log(cin)
  useEffect(() => {
    axios.get(`http://localhost:5000/api/getResvClientCIN/${cin}`)
      .then(res => {
        setResv(res.data);
    ...
    <div className="col-md-12">
      <div className="form-groupProfil">
        {resv
          ...
          .map((row) => (
            <React.Fragment key={row.resvREF}>
                  ...
                  <Grid item xs={8}>
                    ...
                    {new Date(row.startDateResv) > new Date() && (
                      <Grid container justifyContent="flex-end">
                        <Grid item>
                          <IconButton>
                            <button className="btnBlack" onClick={() => onDelete(row.resvREF)}><TbTrash /> Annuler</button>
                          </IconButton>
                        </Grid>
                        <Grid item>
                          <IconButton>
                            <button className="btnBlack" ><a href={`reservation/${row.nom}/${row.nomService}/${row.resvREF}`}><MdUpdate /> Deplacer</a></button>
                          </IconButton>
                        </Grid>
                      </Grid>
                    )}
